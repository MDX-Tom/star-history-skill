...
def build_parser() -> argparse.ArgumentParser:
    ...
    refresh.add_argument(
        "--repository",
        default=os.environ.get("STAR_HISTORY_REPOSITORY"),
        required="STAR_HISTORY_REPOSITORY" not in os.environ,
    )
...
def bootstrap_history(
    *,
    ...
    if current <= created_at:
        current = created_at + timedelta(seconds=1)
    return validate_data(
        {
            "schema_version": SCHEMA_VERSION,
            "repository": repository,
            "updated_at": current.isoformat().replace("+00:00", "Z"),
            "logo_url": download_avatar_data_url(metadata["avatar_url"]),
            "star_records": [
                {"date": format_record_date(created_at), "count": 0},
                {
                    "date": format_record_date(current),
                    "count": metadata["star_count"],
                },
            ],
        },
        repository,
...
def refresh_data_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        metadata = fetch_repository_metadata(repository, token)
        count = metadata["star_count"]
        current_time = datetime.now(timezone.utc)
        if payload.get("bootstrap") is True:
            updated = bootstrap_history(
                repository=repository,
                metadata=metadata,
                current_time=current_time,
            )
            source = "repository bootstrap seed"
        else:
